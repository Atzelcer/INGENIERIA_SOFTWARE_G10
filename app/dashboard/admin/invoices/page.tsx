...
import { FileText, Download, Trash, Eye, Filter } from "lucide-react"
...
export default function AdminInvoicesPage() {
  ...
  const handleSelectAll = (checked: boolean) => {
    setSelectedInvoices(checked ? filteredInvoices.map((invoice) => invoice.id) : [])
  }
  ...
  const handleBulkCancel = () => {
    if (selectedInvoices.length === 0) {
      toast({
        title: "Error",
        description: "Seleccione al menos una factura",
        variant: "destructive",
      })
      return
    }

    const activeInvoices = selectedInvoices.filter((id) => {
      const invoice = invoices.find((inv) => inv.id === id)
      return invoice?.status === "emitida"
    })

    if (activeInvoices.length === 0) {
      toast({
        title: "Error",
        description: "No hay facturas activas seleccionadas para anular",
        variant: "destructive",
      })
      return
    }
    ...
    const updatedInvoices = invoices.map((invoice) =>
      activeInvoices.includes(invoice.id) ? { ...invoice, status: "anulada" as const } : invoice,
    )
    ...
    toast({
      ...
      description: `${activeInvoices.length} facturas han sido anuladas`,
    })
  }
  ...
  const handleBulkDownload = () => {
    ...
    toast({
      ...
      description: `Descargando ${selectedInvoices.length} facturas como ZIP`,
    })
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold tracking-tight">Gestión de Facturas (Admin)</h2>
          <p className="text-muted-foreground">Administración completa de todas las facturas del sistema</p>
        </div>
          ...
          {selectedInvoices.length > 0 && (
            <>
              <Button variant="outline" onClick={handleBulkDownload}>
                <Download className="mr-2 h-4 w-4" />
                Descargar ({selectedInvoices.length})
              </Button>
              <Button variant="destructive" onClick={handleBulkCancel}>
                <Trash className="mr-2 h-4 w-4" />
                Anular ({selectedInvoices.length})
              </Button>
            </>
          )}
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{cancelledInvoices}</div>
        ...
        <TabsContent value={activeTab} className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                {activeTab === "all" && "Todas las facturas del sistema"}
